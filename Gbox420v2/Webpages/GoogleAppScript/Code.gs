...
These files need to be added to Google Sheets, under Tools\Script editor
...
function doGet(espData) {  //ESP-link sends the Logs here through PushingBox. Needs a bit of parsing to make it a proper JSON
  var Log = JSON.parse(espData.parameter.Log);
  ProcessLog(Log);
}
...
function ProcessLog(Log){
  console.log(Log);  //Log the received data on the console output  
...
function goGet_FakeData(){  //For testing only: Pass a properly formated JSON to processing
...
function goGet_FakeData_UpdateAlerts(){  //For testing only: Pass a properly formated JSON to processing
...
function goGet_FakeData_CheckAlerts(){  //For testing only: Pass a properly formated JSON to processing
  var FakeData = {"InternalTemp":"19.80","ExternalTemp":"22.50","InternalHumidity":"46.50","ExternalHumidity":"47.20","InternalFan":"1","ExhaustFan":"0","Light1_Status":"1","Light1_Brightness":"95","LightReading":"95.00","Dark":"0","WaterLevel":"3","WaterTemp":"20.06","PH":"5.96","Pressure":"6.73","Power":"445.00","Energy":"10.00","Voltage":"230","Current":"1.80","Frequency":"-1.00","PowerFactor":"1.00","Light1_Timer":"1","Light1_OnTime":"04:20","Light1_OffTime":"16:20","AeroInterval":"15","AeroDuration":"2"};
...
function SaveToLog(Log) {
  ...
  var dateFormat = getSettingsValue("Date format:");  
...
function CheckAlerts(Log) {
  ...
  var alertEmailEnabled = getSettingsValue("Alert email enabled:") == 'YES';
  var alertEmailAddress = getSettingsValue("Send alerts to:");  
  var alwaysSendEmail = getSettingsValue("Always send email:")  == 'YES'; //Send email when a report is received, even if there are no active/recovered alerts
