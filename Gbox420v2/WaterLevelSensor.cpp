...
WaterLevelSensor::WaterLevelSensor (const __FlashStringHelper * Name, GrowBox * GBox,  Settings::WaterLevelSensorSettings * DefaultSettings): Common(Name){ //constructor
  this -> GBox = GBox;
...
void WaterLevelSensor::refresh_Minute(){
  ...
  if(isAboveSensor1) strcat_P(LevelGauge,(PGM_P)F("#")); else strcat_P(LevelGauge,(PGM_P)F("_")); //Empty
  if(isAboveSensor2) strcat_P(LevelGauge,(PGM_P)F("#")); else strcat_P(LevelGauge,(PGM_P)F("_"));
  if(isAboveSensor3) strcat_P(LevelGauge,(PGM_P)F("#")); else strcat_P(LevelGauge,(PGM_P)F("_"));
  ...
  else if(isAboveSensor1 && isAboveSensor2 && isAboveSensor3 && !isAboveSensor4) Level= 3;
  else if(isAboveSensor1 && isAboveSensor2 && isAboveSensor3 && isAboveSensor4) Level= 4;  //Full
  else Level= -1; //non-valid sensor combination, like: E[#--#]F   
}

void WaterLevelSensor::report(){
  Common::report();
  memset(&LongMessage[0], 0, sizeof(LongMessage));  //clear variable
  strcat_P(LongMessage,(PGM_P)F("Level:")); strcat(LongMessage, getLevelGauge());
  strcat_P(LongMessage,(PGM_P)F(" (")); strcat(LongMessage, getLevelText());strcat_P(LongMessage,(PGM_P)F(")"));
