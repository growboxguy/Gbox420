...
const unsigned long txIntervalMillis = 15000; // send once per 15 seconds
...
const uint8_t WirelessChannel[6] ={"Hemp1"};;
...
void loop() {
    if (millis() - prevMillis >= txIntervalMillis) //Check if it is time to send a command
    {
      sendCommand();
    }
}
