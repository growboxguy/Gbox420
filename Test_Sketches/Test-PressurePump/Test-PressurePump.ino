...
void checkAeroSprayTimer(){
 if(AeroPumpOn)    { //if pump is on
    if(!AeroSolenoidOn && millis() - AeroSprayTimer >= AeroDuration * 1000){  //bypass valve is closed and time to stop spraying (AeroDuration in Seconds)
      AeroPumpOn = false;
      checkRelays();
      ...
      if (AeroSolenoidOn && millis() - AeroPumpTimer >= AeroPumpTimeout * 1000){ //self priming timeout reached, time to start spraying
    ...
    if(millis() - AeroSprayTimer >= AeroInterval * 60000){ //if time to start spraying (AeroInterval in Minutes)
        AeroPumpOn = true;
