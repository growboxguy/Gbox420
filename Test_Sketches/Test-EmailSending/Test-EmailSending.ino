...
void sendEmailAlert(const __FlashStringHelper *title, const __FlashStringHelper *body)
{
  ...
  strcat_P(WebMessage, (PGM_P)F("/pushingbox?devid="));
  strcat(WebMessage, PushingBoxEmailAlertID);
  strcat_P(WebMessage, (PGM_P)F("&Title="));
  strcat_P(WebMessage, (PGM_P)title);
  strcat_P(WebMessage, (PGM_P)F("&Log="));
  strcat_P(WebMessage, (PGM_P)body);
