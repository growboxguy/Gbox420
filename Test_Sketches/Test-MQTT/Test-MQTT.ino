...
void mqttPublish()
{
  ...
  strcat(WebMessage, "{\"Counter\":\"");
  strcat(WebMessage, intToChar(PublishedCounter));
  strcat(WebMessage, "\",\"InternalFan\":\"");
  strcat(WebMessage, intToChar(InternalFan));
  strcat(WebMessage, "\",\"Brightness\":\"");
  strcat(WebMessage, intToChar(LightBrightness));
  strcat(WebMessage, "\",\"LightOn\":\"");
  strcat(WebMessage, intToChar(LightOn)); //bool is int (0 or 1)
  strcat(WebMessage, "\",\"AeroMaxPressure\":\"");
  strcat(WebMessage, floatToChar(AeroMaxPressure));
  strcat(WebMessage, "\",\"MqttString\":\"");
  strcat(WebMessage, TestString);
  strcat(WebMessage, "\"}"); //closing the JSON

  memset(&MqttPath[0], 0, sizeof(MqttPath)); //reset variable to store the Publish to path
  strcat(MqttPath, MqttTopic);
  Serial.print(PublishedCounter);
...
void mqttReceived(void *response)
{
  ...
  if (strstr(topic, MqttBrightness) != NULL)
  {
    setBrightness(atoi(data));
  } //expects int parameter
  else if (strstr(topic, MqttLightOn) != NULL)
  {
    setLightOnOff(atoi(data));
  } //expects bool parameter
  else if (strstr(topic, MqttInternalFan) != NULL)
  {
    setFanSpeed(atoi(data));
  } //expects int parameter, 3 options: 0-Off,1-Low,2-High
  else if (strstr(topic, MqttAeroMaxPressure) != NULL)
  {
    setAeroMaxPressure(atof(data));
  } //expects float parameter
  else if (strstr(topic, MqttString) != NULL)
  {
    setMqttSting(data);
  } //expects char array parameter
}
...
void setMqttSting(char *StringToSet)
{
  ...
  Serial.print("Word of the day is: ");
  Serial.println(StringToSet);
}
