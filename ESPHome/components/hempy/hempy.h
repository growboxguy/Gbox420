...
namespace esphome
{
  namespace hempy
  {
    ...
    public:
      ...
      HempyBucket(std::string name, text_sensor::TextSensor *state_sensor, sensor::Sensor *weight_sensor, number::Number *start_watering_weight, number::Number *watering_increments, number::Number *max_watering_weight, number::Number *max_watering_time, number::Number *drain_wait_time, number::Number *drain_target_weight, number::Number *evaporation_target_weight, sensor::Sensor *next_watering_weight, switch_::Switch *waterPump) : PollingComponent(UpdateIntervalMS), Name(name), StateSensor(state_sensor), WeightSensor(weight_sensor), StartWateringWeight(start_watering_weight), WateringIncrements(watering_increments), MaxWateringWeight(max_watering_weight), MaxWateringTime(max_watering_time), DrainWaitTime(drain_wait_time), DrainTargetWeight(drain_target_weight), EvaporationTargetWeight(evaporation_target_weight), NextWateringWeight(next_watering_weight), WaterPump(waterPump) {}
      void setup() override;
