...
err_t recv_callback(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    if (err == ERR_OK && p != NULL) {
    	...
    	case MQTT_MSGT_PUBLISH:
    		...
    		printf("Publish in\n");
    		Storage->msgReceivedCallback(Storage, topic, topicLen, data, dataLen);
    		break;
...
void mqttDisconnectForced(Mqtt *Storage)
{
	...
	//tcp_close(Storage->pcb);
	tcp_abort(Storage->pcb);

	Storage->connected = 0;
}


void mqttInit(Mqtt *Storage, struct ip_addr_t serverIp, int port, msgReceived fn, char *devId){
	Storage->msgReceivedCallback = fn;
	//Storage->pcb = tcp_new();
...
http_poll(void *arg, struct tcp_pcb *pcb)
{
  ...
  if ((hs == NULL) && (pcb->state == ESTABLISHED)) {
	...
	if(!Storage->connected && Storage->pollAbortCounter == 4)
	{
...
uint8_t mqttConnect(Mqtt *Storage) {
    ...
    // variable header
    uint8_t var_header[] = {0x00,0x06,0x4d,0x51,0x49,0x73,0x64,0x70,0x03,0x02,0x00,KEEPALIVE/1000,0x00,strlen(Storage->deviceId)};

    // fixed header: 2 bytes, big endian
    uint8_t fixed_header[] = {MQTTCONNECT,12+strlen(Storage->deviceId)+2};

    char packet[sizeof(fixed_header)+sizeof(var_header)+strlen(Storage->deviceId)];
    ...
    // Copy device name
    memcpy(packet+sizeof(fixed_header)+sizeof(var_header),Storage->deviceId,strlen(Storage->deviceId));

    //Send MQTT identification message to broker.
    err = tcp_write(Storage->pcb, (void *)packet, sizeof(packet), 1);

    if (err == ERR_OK) {
        tcp_output(Storage->pcb);
        Storage->lastActivity = msTime;
        //Storage->connected = 1;
...
uint8_t mqttPublish(Mqtt *Storage,char* pub_topic, char* msg) {
	...
	if(!Storage->connected)
	{
		printf("Publish err:no Storage->connected");
		return 1;
	}
    ...
    //Publish message
    err_t err = tcp_write(Storage->pcb, (void *)packet_pub, sizeof(packet_pub), 1); //TCP_WRITE_FLAG_MORE
...
uint8_t mqttDisconnect(Mqtt *Storage) {
    ...
    tcp_write(Storage->pcb, (void *)packet_224, sizeof(packet_224), 1);
    tcp_write(Storage->pcb, (void *)(0), sizeof((0)), 1);
    //socket->send((char*)224,sizeof((char*)224));
    //socket->send((uint8_t)0,sizeof((uint8_t)0));
    tcp_close(Storage->pcb);
    //Storage->lastActivity = timer.read_ms();
    Storage->connected = 0;
...
uint8_t mqttSubscribe(Mqtt *Storage, char* topic) {
    ...
    if (!Storage->connected)
        ...
        //Send message
        err_t err = tcp_write(Storage->pcb, (void *)packet_topic, sizeof(packet_topic), 1); //TCP_WRITE_FLAG_MORE
        if (err == ERR_OK) {
            tcp_output(Storage->pcb);
...
int mqttPing(Mqtt *Storage)
{
    ...
    //Publish message
    err_t err = tcp_write(Storage->pcb, (void *)&pingReq, sizeof(pingReq), 1); //TCP_WRITE_FLAG_MORE
...
uint8_t mqttLive(Mqtt *Storage) {
	...
	uint32_t t = msTime;
	if (t - Storage->lastActivity > (KEEPALIVE - 2000)) {

		if (Storage->connected) {
			printf("Sending keep-alive\n");
			mqttPing(Storage);
		} else if(Storage->autoConnect){
			mqttConnect(Storage);
		}

		Storage->lastActivity = t;
	}
