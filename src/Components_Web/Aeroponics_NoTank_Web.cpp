...
void Aeroponics_NoTank_Web::websiteEvent_Button(char *Button)
{
  ...
  {
    if (strcmp_P(ShortMessage, (PGM_P)F("BypassOn")) == 0){setBypassOn(true);}
    else if (strcmp_P(ShortMessage, (PGM_P)F("BypassOff")) == 0){setBypassOff(true);}    
...
{ ///When a field is submitted using the Set button
  if (strcmp_P(ShortMessage, (PGM_P)F("Timeout")) == 0){Pump->setTimeOut(WebServer.getArgInt());}
