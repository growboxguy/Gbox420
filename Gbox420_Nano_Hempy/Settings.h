...
static const uint8_t Version = 8; ///< Increment this when you make a change in the SAVED TO EEPROM secton
...
///SAVED TO EEPROM - Before uploading the schetch increase the Version variable to override whatever is stored in the Arduino's EEPROM 
  typedef struct
  {
   ...
   struct WaterPumpSettings
    ...
    struct WaterPumpSettings HempyPump1 = {.PumpPin = 3, .PumpPinNegativeLogic = false, .PumpEnabled = true, .PumpTimeOut = 420}; ///< Pumps do not need a bypass solenoid
    struct WaterPumpSettings HempyPump2 = {.PumpPin = 4, .PumpPinNegativeLogic = false, .PumpEnabled = true, .PumpTimeOut = 420}; ///< Pumps do not need a bypass solenoid
    ...
    struct WeightSensorSettings Weight1 = {.DTPin = 5, .SCKPin = 6, .Offset = -121275, .Scale = -126032.00}; ///Update the calibration values here for Weight Sensor 1
